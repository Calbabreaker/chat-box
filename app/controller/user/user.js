const { vald, startChain } = require(global.rootDir + "/app/middleware/validator");
...
router.post("/signup", async (req, res) => {
    try {
        ...
        const check = await usersDatabase.checkProperty({
        ...
        const errors = await startChain(data)
            ...
            .ensure("password", "Password is invalid")
...
router.put("/usericon", async (req, res) => {
    try {
        ...
        const errors = await startChain(req.files)
            .check("usericon")
        ...
        await sharp(req.files.usericon.data)
